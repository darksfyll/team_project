	...
	<div id="wrap">
		...
		<section>
			<div>
				<h1>주변기기&#38;악세사리</h1>
				<ul id="category">
					<li><a href="category?productCategory=Etc">신상품</a></li>
					<li><a href="orderBy?productCategory=Etc&where=product_views&order=desc">인기상품</a></li>
					<li><a href="orderBy?productCategory=Etc&where=product_price&order=asc">최저가</a></li>
					<li><a href="orderBy?productCategory=Etc&where=product_price&order=desc">최고가</a></li>
				...
				<div style="width:800px; text-align: center;">
					<c:forEach var="pgn" items="${pagination}">
						<c:choose>
							<c:when test="${views == 1 }">
								<a href="orderBy?productCategory=Etc&where=product_views&order=desc&page=${pgn.pageNum }">
									<c:choose>
							...
							<c:when test="${lowest == 1 }">
								<a href="orderBy?productCategory=Etc&where=product_price&order=asc&page=${pgn.pageNum }">
									<c:choose>
										<c:when test="${pgn.curPage }">
											<u>${pgn.display }</u>
										</c:when>
							...
							<c:when test="${highest == 1 }">
								<a href="orderBy?productCategory=Etc&where=product_price&order=desc&page=${pgn.pageNum }">
									<c:choose>
										<c:when test="${pgn.curPage }">
											<u>${pgn.display }</u>
										</c:when>
							...
							<c:otherwise>
								<a href="category?productCategory=Etc&page=${pgn.pageNum }">
									<c:choose>
										<c:when test="${pgn.curPage }">
											<u>${pgn.display }</u>
										</c:when>
