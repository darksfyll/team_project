			...
			<div>
				...
				<div style="width:800px; text-align: center;">
					<c:forEach var="pgn" items="${pagination}">
						<c:choose>
							<c:when test="${views == 1 }">
								<a href="orderBy?productCategory=Office&where=product_views&order=desc&page=${pgn.pageNum }">
									<c:choose>
							...
							<c:when test="${lowest == 1 }">
								<a href="orderBy?productCategory=Office&where=product_price&order=asc&page=${pgn.pageNum }">
									<c:choose>
										<c:when test="${pgn.curPage }">
											<u>${pgn.display }</u>
										</c:when>
							...
							<c:when test="${highest == 1 }">
								<a href="orderBy?productCategory=Office&where=product_price&order=desc&page=${pgn.pageNum }">
									<c:choose>
										<c:when test="${pgn.curPage }">
											<u>${pgn.display }</u>
										</c:when>
							...
							<c:otherwise>
								<a href="category?productCategory=Office&page=${pgn.pageNum }">
									<c:choose>
										<c:when test="${pgn.curPage }">
											<u>${pgn.display }</u>
										</c:when>
